...
Application web PHP/HTML/CSS/JS reliée à PostgreSQL et aux scripts Python du projet BigData/IA.
...
- `scripts/` : utilitaires CLI (import CSV)

## Prérequis

- PHP 8.1+ avec extensions `pdo` et `pdo_pgsql`
- PostgreSQL 13+
...
export PGUSER=sq_web_user
export PGPASSWORD=sq_web_password
```
...
- `Connexion PostgreSQL indisponible` : vérifier `PGHOST/PGPORT/PGDATABASE/PGUSER/PGPASSWORD`.
- `could not find driver` : extension `pdo_pgsql` absente.
